-- This script uses AdventureWorks2014, but it really could use any database as it creates and maintains its own set of objects
...
-- ***************************
-- Jump to here to skip initial table creation
-- ***************************

SELECT TOP 100 ReportID, ReportName, ReportNumber, LenOfReportDescription = LEN(ReportDescription), ReportDescription FROM CodeCamp.Reports

SELECT		SUM(Cast(DATALENGTH(ReportID) as BigInt)) as [ReportID_Size]
		,	SUM(Cast(DATALENGTH(ReportName) as BigInt)) as [ReportName_Size]
		,	SUM(Cast(DATALENGTH(ReportNumber) as BigInt)) as [ReportNumber_Size]
		,	SUM(Cast(DATALENGTH(ReportDescription) as BigInt)) as [ReportDescription_Size]
		,	SUM(Cast(DATALENGTH(ReportDescription) as BigInt) + 0.0) / SUM(Cast(DATALENGTH(ReportNumber) as BigInt)) as [ReportDescription_BiggerThan_ReportNumber]
FROM		CodeCamp.Reports
